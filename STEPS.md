...
export const BrutalButton = ({
  ...
  const variants: Record<Variant, string> = {
    solid: 'bg-acid text-ink shadow-[var(--shadow-brutal)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none',
    outline: 'bg-transparent text-ink hover:bg-ink hover:text-bone',
    danger: 'bg-orange text-ink shadow-[var(--shadow-brutal)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none',
  };
...
export const LoginPage = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-[480px] border-2 border-ink shadow-[var(--shadow-brutal)] p-8 mt-16">
...
### A) Initialize the App (`fly launch`)

From the **repo root** (so the Dockerfile sees the workspace structure):
...
The CLI will ask:
- **App name:** `fragment-api` (or `fragment-api-<yourname>`)
- **Region:** `fra` / `iad` / closest to Atlas region.
- **Postgres / Redis:** **No** for both.
- **Deploy now:** **No**.

This generates `fly.toml` at the repo root. Replace with:
...
```bash
fly volumes create fragment_data --region fra --size 3
fly volumes list
```

### C) Set Production Secrets

```bash
fly secrets set \
  MONGO_URI="<paste your full Atlas SRV string from STEP 39>" \
  JWT_SECRET="$(openssl rand -hex 32)" \
  JWT_EXPIRES_IN="7d" \
  BCRYPT_SALT_ROUNDS="12" \
  CLIENT_ORIGIN="https://fragment.netlify.app" \
...
> **Windows JWT_SECRET:** `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`.
...
The build runs the multi-stage Dockerfile remotely. Watch for:
- `npm ci` (full install)
...
```bash
fly status
fly logs                                          # look for [DB] connected
curl https://fragment-api.fly.dev/api/health
fly ssh console -C "ffmpeg -version"
```

### E) Seed the Admin User

```bash
fly ssh console -C "node /app/server/dist/seed/seedAdmin.js"
```

(Note the compiled-JS path — `tsx` is not available in production.)

### F) Backend Operational Verification

- [ ] `fly status` shows machine running, volume mounted at `/data`.
- [ ] `fly ssh console -C "df -h /data"` reports volume mounted with ~3 GB free.
- [ ] `fly logs` shows `[DB] connected` and the Express startup banner.
- [ ] `fly ssh console -C "ffmpeg -version"` prints FFmpeg version.
- [ ] `curl https://fragment-api.fly.dev/api/health` returns `{ status: "ok", ... }`.
- [ ] Login as the seeded admin returns a JWT token.

**Exit criteria for STEP 41:**
- Backend reachable at `https://fragment-api.fly.dev/api/health`.
- Admin user exists in the Atlas DB.
- All operational checks pass.

**SECURITY:**
- All secrets injected via `fly secrets set` are encrypted at rest.
- `force_https = true` means `http://` is auto-redirected to `https://` at the edge.
- `auto_stop_machines = false` means the machine never sleeps.
- The persistent volume is encrypted at rest.
- Admin seed password is set via secret.
- Multi-stage Docker build means TypeScript compiler is NOT present in the runtime image — one fewer dependency to patch.
